...
def courseView_students(window, return_function,teacher,person,course):#this is going to show the course the teacher is currently teaching
	...
	for i in range (len(studentIncourse)):
		...
		lastName_label = Label(window, text = (str(person.fname) + " " + str(person.lname)),fg = "#00293c", font = "Arial 13",relief="ridge",borderwidth=2)
	...
	return_course = Button(window,text = "Return", fg = "#2E4053", font = "Arial 15 bold",bg = "#48C9B0", highlightbackground ="#48C9B0", height = 1,width = 10,borderwidth=2,cursor = get_handcursor(), relief="groove", command = partial(ui.TeacherCourses.course,window, return_function, teacher, person))

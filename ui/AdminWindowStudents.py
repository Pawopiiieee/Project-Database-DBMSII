...
def admin_window_students(window,return_function):
	...
	for i in range (len(last_name_students)):
		...
		view_button = Button(window, text = "View",font = "Arial 10", fg = "#006386",bg = "#ccd9ff",highlightbackground = "#ccd9ff",cursor = get_handcursor(), height = 1,width = 4, relief = FLAT,command = partial(student_view, window, return_function))
		view_button.place(x= 330, y = y_position, height=26)
		edit_button = Button(window, text = "Edit",font = "Arial 10", fg = "#006386",bg = "#fff2cc",highlightbackground = "#fff2cc",cursor = get_handcursor(), height = 1,width = 4, relief = FLAT, command = partial(student_edit, window, return_function))
